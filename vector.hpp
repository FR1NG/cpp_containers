...
namespace ft {
    ...
    public:
        ...
        explicit vector(const Allocator &alloc) : _allocator(alloc), _v(NULL), _size(0), _capacity(0) {}

        template<class InputIt>
        vector(InputIt first, InputIt last, const Allocator &alloc = Allocator(),
               typename enable_if<!is_integral<InputIt>::value, InputIt>::type =
               InputIt()): _allocator(alloc), _v(NULL), _size(0), _capacity(0) {
...
//            ! need to back to ti lather to handle assignment of greater size vector
            if (&vec == this)
                return (*this);
      ...
      void reserve(size_type n)
      {
        ...
        iterator insert(iterator position, const value_type &val) {
            if (position == this->end() && !this->empty()) {
              ...
              for (; i > 0 && i > stop; i--) {
                        this->_allocator.destroy(this->_v + i);
                        this->_allocator.construct(this->_v + i,
                                                   this->_v[i - 1]);
              }
              this->_allocator.construct(this->_v + i, val);
              this->push_back(tmp);
        ...
        void insert (iterator position, InputIterator first, InputIterator last)
        {
            ...
            else
            {
              ...
              while(copy_from >= stop)
              {
                ...
                this->_allocator.construct(this->_v + copy_to, *(this->begin() + copy_from));
                copy_from--;
                copy_to--;
              }
